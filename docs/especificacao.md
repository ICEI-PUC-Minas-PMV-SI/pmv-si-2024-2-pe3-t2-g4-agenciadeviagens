...
Fluxo Principal:
O Usuário acessa a funcionalidade de Gerenciar Viagens.
O Sistema apresenta as três opções disponíveis: Gerenciar Viagens Futuras, Gerenciar Pacotes de Viagem Favoritos ou Verificar Viagens Concluídas.
O Usuário seleciona a operação desejada.
O Sistema apresenta as informações relacionadas à opção selecionada.
O Usuário pode realizar ações de inclusão, exclusão, alteração ou consulta relacionadas à operação escolhida, ou optar por encerrar o caso de uso.
Se o Usuário desejar realizar outra operação, o caso de uso retorna ao passo 2; caso contrário, o caso de uso termina.
Fluxo Alternativo (3): Gerenciar Viagens Futuras
...
Fluxo Alternativo (3): Gerenciar Pacotes de Viagem Favoritos
...
Fluxo Principal:
O Usuário acessa a funcionalidade de busca de pacotes de viagem.
O Sistema solicita que o usuário forneça critérios de busca, como destino, data, duração, ou faixa de preço.
O Usuário insere os critérios desejados e confirma a busca.
O Sistema processa os critérios e apresenta uma lista de pacotes de viagem que correspondem aos parâmetros fornecidos.
O Usuário pode selecionar um pacote para visualizar os detalhes.
O Sistema exibe os detalhes completos do pacote de viagem, incluindo destino, data, preço, disponibilidade, descrição e condições.
O Usuário pode optar por realizar outra busca, reservar a viagem ou encerrar o caso de uso.
Fluxo Alternativo (3): Busca sem Resultados
...
Fluxo Alternativo (5): Pacote Indisponível
...
Fluxo Principal:
O Gerente de Agência requisita a manutenção dos usuários.
O Sistema apresenta as operações disponíveis: inclusão de um novo usuário, alteração de um usuário existente, exclusão de um usuário ou consulta dos usuários cadastrados.
O Gerente de Agência seleciona a operação desejada: Inclusão, Exclusão, Alteração ou Consulta, ou opta por finalizar o caso de uso.
...
Fluxo Principal:
O Gerente de Agência requisita a manutenção de pacotes de viagens.
O Sistema apresenta as operações disponíveis: inclusão de um novo pacote, alteração de um pacote existente, exclusão de um pacote ou consulta de pacotes cadastrados.
O Gerente de Agência seleciona a operação desejada: Inclusão, Exclusão, Alteração ou Consulta, ou opta por finalizar o caso de uso.
...
Fluxo Principal:
O Gerente de Agência requisita a manutenção de fornecedores.
O Sistema apresenta as operações disponíveis: inclusão de um novo fornecedor, alteração de um fornecedor, exclusão de um fornecedor ou consulta de dados de um fornecedor.
O Gerente de Agência seleciona a operação desejada: Inclusão, Exclusão, Alteração ou Consulta, ou opta por finalizar o caso de uso.
